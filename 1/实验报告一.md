...
当有多个输入时，对优先级最高的进行编码，其它低位对编码结果无影响。
...
输入的十进制数用对应的信号灯亮起表示，例如输入是$6$，第$2$个灯亮起，输入是$4$，第$4$个灯亮起。
...
输出为$3$位，第一位高电平表示第一个输入大，第三位高电平表示第二个输入较大，第二位高电平表示两个输入相等；输入为两个$4$位二进制。
